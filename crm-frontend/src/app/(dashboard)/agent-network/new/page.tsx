...
function AgentForm() {
  ...
  useEffect(() => {
    if (editId) {
      apiFetch(API_URL + "/agents/" + editId)
        .then((res) => res.json())
        .then((result) => {
          if (result.success) {
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500 px-2.5 md:px-0 mt-2 md:mt-0 mb-20 md:mb-0">
      ...
      <form onSubmit={handleAddSubmit} className="space-y-6">
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 sm:gap-4 pt-4 pb-4 md:pb-0">
          <Button type="button" variant="ghost" className="h-12 px-8 rounded-xl font-medium text-[15px] w-full sm:w-auto" onClick={() => router.push("/agent-network")} disabled={isSubmitting}>
            Cancel
          </Button>
